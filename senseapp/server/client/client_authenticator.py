...
class ClientAuthenticator:
    ...
    def __init__(self, server_credentials: ServerCredentials):
        ...
        self.encoded_credentials = base64.b64encode(
            f"{self.user_name}:{self.password}".encode("utf-8")).decode('utf-8')

    async def authenticate(self, message: str, websocket: WebSocketClientProtocol) -> Optional[SenseClient]:
        ...
        try:
            ...
            await websocket.send(AuthenticationRespone(auth_result=AuthResult.FAILURE).model_dump_json(exclude_none=True))
            return None

        if self._is_authenticated(auth_message):
            sense_client = SenseClient(websocket)
            sense_client.set_client_data(auth_message)
            await websocket.send(AuthenticationRespone(auth_result=AuthResult.SUCCESS).model_dump_json(exclude_none=True))
            return sense_client
        else:
            await websocket.send(AuthenticationRespone(auth_result=AuthResult.FAILURE).model_dump_json(exclude_none=True))
            return None
